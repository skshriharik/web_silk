...
/*  $user_check_query = "SELECT * FROM producer WHERE mobile='$ph_number'";
  ...
  if(mysqli_query($connect,$perm_query))
    ...
    echo '  insidded ';
    $file1=addslashes(file_get_contents($_FILES["cert"]["tmp_name"]));
  //  $file2=addslashes(file_get_contents($_FILES["sign"]["tmp_name"]));
    $query = "UPDATE renew_producer SET reg_cert_photo = '$file1', exp_date=ADDDATE(CURDATE(),90)  WHERE mobile= '$ph_number'";
    ...
    /*  $del_query="DELETE FROM producer WHERE mobile='$ph_number'";
      ...
      header('location: renew_pdf_generate.php');
}
